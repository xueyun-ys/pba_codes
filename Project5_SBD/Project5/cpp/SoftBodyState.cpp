...
{//urc:upper right ...?
	...
	for (int j = 0; j < nz; j++)
	{
		for (int i = 0; i < nx; i++)
		{
			s->add_bend(i + j*nx + j, i + j*nx + j + nx + 1, i + j*nx + j + 1,  i + j*nx + j + nx + 1 + 1);
		}
	}
...
{
	// a = ((pos(i) - pos(j)) ^ (pos(k) - pos(j))).magnitude();
...
void pba::SoftBodyStateData::add_bend(size_t i, size_t j, size_t k, size_t l)
{
	...
	double costheta = 1.0 * n0 * n1;//+-1
	Vector h = (pos(k) - pos(j)) / (pos(k) - pos(j)).magnitude();
	double sintheta = -1.0 * h * (n0 ^ n1);//+-1
