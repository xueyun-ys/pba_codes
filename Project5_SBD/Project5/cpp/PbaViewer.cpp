...
void cbIdleFunc()
{
   PbaViewer::Instance() -> Idle();//start from here~~~~~~~~~调用了PBAviewer的方法生成对象再调用该对象的idle
}
...
void PbaViewer::Init( const std::vector<std::string>& args )//main方法传进来的参数
{
   int argc = (int)args.size();
   char** argv = new char*[argc];
   for( int i=0;i<argc;i++)
   {
      argv[i] = new char[args[i].length() + 1];//基本上就是把传入的参数存储到argv中，存之前是字符串（一句话），存之后是二维数组。
   ...
   {
      things[i]->Init(args);//初始化各个things
   }
   initialized = true;
   ...
   /*CollisionSurface surf = makeCollisionSurface();
	...
	surf->get_triangle(4)->set_invisable();
	surf->get_triangle(5)->set_invisable();*/
	//=====================================================
...
void PbaViewer::Idle()
{
   for( size_t i=0;i<things.size();i++)
   {
      things[i]->Idle();//1.调用各个things的idle
   }
}
