...
std::vector< std::vector<int> > faces2;
...
namespace pba 
{
	...
	pba::CollisionSurface buildcube(double s)
	{
		verts[0] = pba::Vector(-1, -1, -1)*s;
		verts[1] = pba::Vector(1, -1, -1)*s;
		verts[2] = pba::Vector(1, 1, -1)*s;
		verts[3] = pba::Vector(-1, 1, -1)*s;
		verts[4] = pba::Vector(-1, -1, 1)*s;
		verts[5] = pba::Vector(1, -1, 1)*s;
		verts[6] = pba::Vector(1, 1, 1)*s;
		verts[7] = pba::Vector(-1, 1, 1)*s;
		...
		//==========================================create ground===========================================
		verts[0] = pba::Vector(-100, -1, -100)*s;
		verts[1] = pba::Vector(100, -1, 1000)*s;
		verts[2] = pba::Vector(100, -1, -1000)*s;


		normals[0] = pba::Vector(0, 1, 0);


		face_colors[0] = pba::Color(0.5, 0.7, 0.7, 0);
		...
		{
			std::vector<int>& face2 = faces2[i];

			CollisionTriangle tri1 = makeCollisionTriangle(Vertex(face2[0]), Vertex(face2[1]), Vertex(face2[2]));//构造new一个新的collision triangle对象
			tri1->set_color(face_colors[i]);
			surf->addTriangle(tri1);
		}
		//================================================================================================
	...
	CollisionSurface buildground(double s)
	{
		verts[0] = pba::Vector(-100, 0, 0)*s;
		verts[1] = pba::Vector(100, 100, 0)*s;
		verts[2] = pba::Vector(100, -100, 0)*s;
