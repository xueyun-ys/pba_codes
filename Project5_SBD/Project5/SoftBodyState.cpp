...
{//urc:upper right ...?
	...
	for (int j=0; j < nz+1; j++) 
	{
		...
		{
			s->set_pos(i, Vector(llc.X() + stepx*i, urc.Y(), urc.Z()-stepz*j));
		}
	}
	//add pairs
	for (int j = 0; j < nz + 1; j++)
	{
		for (int i = 0; i < nx+1; i++)
		{
			if (i < nx)
			{
				s->add_pair(i + j*nx, i + j*nx + 1);
				if (j < nz)
				{
					s->add_pair(i + j*nx, i + j*nx + 1+nx);
				}
			}
			if (j < nz)
			{
				s->add_pair(i + j*nx, i + j*nx + nx);
				if (i > 0)
				{
					s->add_pair(i + j*nx, i + j*nx - 1 + nx);
				}
	...
	for (int j = 0; j < nz + 1 ; j++)
	{
		for (int i = 0; i < nx + 1; i++)
		{
			if (i < nx && j < nz)
			{
				s->add_triangle(i + j*nx, i + j*nx +1, i + j*nx + nx);
				s->add_triangle(i + j*nx, i + j*nx + 1, i + j*nx + nx + 1);
				s->add_triangle(i + j*nx, i + j*nx + nx, i + j*nx + nx + 1);
				s->add_triangle(i + j*nx +1, i + j*nx + nx, i + j*nx + nx + 1);
			}
		}
	}
	...
	for (int j = 0; j < nz; j++)
	{
		for (int i = 0; i < nx; i++)
		{
			s->add_bend(i + j*nx, i + j*nx + 1, i + j*nx + nx, i + j*nx + nx + 1);
		}
	}
