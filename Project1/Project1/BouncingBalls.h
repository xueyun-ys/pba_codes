...
namespace pba {
	...
	public:
		...
		////==========================================================
			...
			surf->get_triangle(4)->set_invisible();
			surf->get_triangle(5)->set_invisible();
			...
			//emitter(ParticleEmitter(Vector(0, 0, 0), Vector(0, 0, 0), 0.25, 1.0))
		{
			...
			state->add(1);
			//state->set_pos(0, Vector(2.0, 1., 1.));//改变位置
			state->set_vel(0, Vector(0.0,0,0));
		...
		void Display()
		{
			...
			{
				if (!box->get_triangle(i)->visibility())
					continue;

				pba::Color color = box->get_triangle(i)->get_color();
				glColor4f(color.red(), color.green(), color.blue(), color.alpha());

				pba::Vector v = box->get_triangle(i)->vertex(0);
				glVertex3f(v.X(), v.Y(), v.Z());
				glNormal3f(v.X(), v.Y(), v.Z());

				v = box->get_triangle(i)->vertex(1);
				glVertex3f(v.X(), v.Y(), v.Z());
				glNormal3f(v.X(), v.Y(), v.Z());

				v = box->get_triangle(i)->vertex(2);
				glVertex3f(v.X(), v.Y(), v.Z());
				glNormal3f(v.X(), v.Y(), v.Z());
			}
			glEnd();
		...
		void Keyboard(unsigned char key, int x, int y)
		{
			...
			if (key == '1')
			{
				verts[0] = pba::Vector(-0.2, -0.2, -0.2);
				verts[1] = pba::Vector(0.2, -0.2, -0.2);
				verts[2] = pba::Vector(0.2, 0.2, -0.2);
				verts[3] = pba::Vector(-0.2, 0.2, -0.2);
				verts[4] = pba::Vector(-0.2, -0.2, 0.2);
				verts[5] = pba::Vector(0.2, -0.2, 0.2);
				verts[6] = pba::Vector(0.2, 0.2, 0.2);
				verts[7] = pba::Vector(-0.2, 0.2, 0.2);
				...
				face_colors[0] = pba::Color(0.2, 0.5, 0.7, 0);
				face_colors[1] = pba::Color(0.8, 0.5, 0.1, 0);
				face_colors[2] = pba::Color(0.4, 0.5, 0.9, 0);
				face_colors[3] = pba::Color(0.6, 0.8, 0.2, 0);
				face_colors[4] = pba::Color(0.4, 0.4, 0.7, 0);
				face_colors[5] = pba::Color(0.2, 0.7, 0.1, 0);
				...
				for (size_t i = 0; i < faces.size(); i++)
				{
					...
					CollisionTriangle tri1 = makeCollisionTriangle(Vertex(face[0]), Vertex(face[1]), Vertex(face[2]));//构造new一个新的collision triangle对象
					tri1->set_color(face_colors[i]);
					box->addTriangle(tri1);

					CollisionTriangle tri2 = makeCollisionTriangle(Vertex(face[2]), Vertex(face[3]), Vertex(face[0]));
					int index = (i + 1) % 6;
					tri2->set_color(face_colors[(i + 1) % 6]);
					box->addTriangle(tri2);
				}
...
#pragma omp critical
					...
					P = Vector(drand48(), drand48(), drand48());
					V = Vector((drand48()-0.5)*2, 0.0, (drand48() - 0.5) * 2);
					C[0] = 0.8 + 0.2*C[0];
					state->set_pos(i, P);
					state->set_vel(i, V);
					state->set_ci(i, Color(drand48(), drand48(), drand48(),1));
				}
				emit = false;
			}
...
//#pragma omp critical
				P = Vector(drand48(), drand48(), drand48());
				V = Vector((drand48() - 0.5), 0.0, (drand48() - 0.5));
				C[0] = 0.8 + 0.2*C[0];
				state->set_pos(i, P);
				state->set_vel(i, V);
				state->set_ci(i, Color(drand48(), drand48(), drand48(), 1));
